...
The $\approx 81\%$ of variance in $\tM$ explained by $|\Delta\Delta\mathcal{S}|$ alone
($\rho^2 = 0.81$) leaves a residual that we attribute to pair-specific quasiparticle
dynamics, not to DQPT timing.
...
$\gc = 0.5$) to the XX model ($\gamma = 1$, $\gc = 0$)
...
The critical test of DQPT independence uses $\gf = 0.42$, which lies in the FM phase
for the TFIM and $\gamma = 0.25$ (no DQPTs), but in the PM phase for $\gamma \geq 0.5$
(DQPTs present, since $\gf > \gc(\gamma)$ for those values).
QME is observed for all five $\gamma$ values at this field.
This single experiment — within which the FM/PM classification changes as
$\gamma$ is tuned while all other parameters are fixed — provides direct,
side-by-side evidence that DQPT presence is not required for Z2 QME.
